...
impl MapServer {
    ...
    /// 处理存款请求 (0x09B9)
    pub(super) fn handle_bank_deposit(
        &self,
        data: &[u8],
        session: &mut Session,
    ) -> Option<Vec<u8>> {
        ...
        tracing::info!("Player {} 存款: {} zeny", player_id, pkt.amount);
    ...
    /// 处理取款请求 (0x09BA)
    pub(super) fn handle_bank_withdraw(
        &self,
        data: &[u8],
        session: &mut Session,
    ) -> Option<Vec<u8>> {
        ...
        tracing::info!("Player {} 取款: {} zeny", player_id, pkt.amount);
    ...
    /// 处理打开商城请求 (0x0845)
    pub(super) fn handle_cash_shop_open(
        &self,
        _data: &[u8],
        session: &mut Session,
    ) -> Option<Vec<u8>> {
    ...
    /// 处理购买商城物品请求 (0x0848)
    pub(super) fn handle_cash_shop_buy(
        &self,
        data: &[u8],
        session: &mut Session,
    ) -> Option<Vec<u8>> {
    ...
    /// 处理关闭商城请求 (0x084A)
    pub(super) fn handle_cash_shop_close(
        &self,
        _data: &[u8],
        session: &mut Session,
    ) -> Option<Vec<u8>> {
