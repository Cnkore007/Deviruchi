...
impl MapServer {
    /// 处理添加好友请求 (0x0201)
    pub(super) fn handle_friends_list_add(&self, data: &[u8], session: &mut Session) -> Option<Vec<u8>> {
        let player_id = session.player_id?;
        let pkt = CzFriendsListAdd::from_slice(data)?;

        tracing::info!(
            "Player {} 请求添加好友: char_id={}",
            player_id, pkt.char_id
        );
    ...
    /// 处理删除好友请求 (0x0203)
    pub(super) fn handle_friends_list_remove(&self, data: &[u8], session: &mut Session) -> Option<Vec<u8>> {
        let player_id = session.player_id?;
        let pkt = CzFriendsListRemove::from_slice(data)?;

        tracing::info!(
            "Player {} 请求删除好友: char_id={}",
            player_id, pkt.char_id
        );
    ...
    /// 处理好友请求回复 (0x0208)
    pub(super) fn handle_friends_list_reply(&self, data: &[u8], session: &mut Session) -> Option<Vec<u8>> {
        let player_id = session.player_id?;
        let pkt = CzFriendsListReply::from_slice(data)?;

        tracing::info!(
            "Player {} 回复好友请求: char_id={}, reply={}",
            player_id, pkt.char_id, pkt.reply
        );
