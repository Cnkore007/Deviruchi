...
use crate::protocol::map_packets::{
    CzInsertCard, CzItemIdentify, CzWeaponRefine,
    ZcItemIdentifyAck, ZcWeaponRefineAck,
};
...
impl MapServer {
    ...
    pub(super) fn handle_insert_card(&self, data: &[u8], session: &mut Session) -> Option<Vec<u8>> {
        ...
        tracing::info!(
            ...
            player_id, pkt.card_index, pkt.equip_index
        );
    ...
    /// 处理鉴定物品请求 (0x01DD)
    pub(super) fn handle_item_identify(&self, data: &[u8], session: &mut Session) -> Option<Vec<u8>> {
        let player_id = session.player_id?;
    ...
    /// 处理精炼武器请求 (0x0222)
    pub(super) fn handle_weapon_refine(&self, data: &[u8], session: &mut Session) -> Option<Vec<u8>> {
        let player_id = session.player_id?;
        ...
        if let Some(item) = inventory.get_mut(pkt.index as usize) {
            if success && item.refine < 10 {
                item.refine += 1;
                tracing::info!("Player {} 精炼成功，当前精炼等级: {}", player_id, item.refine);
                Some(ZcWeaponRefineAck {
                    result: 0, // 成功
                    index: pkt.index,
                }.to_packet())
