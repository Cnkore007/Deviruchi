...
> **50,000+ 行** Rust 代码，180 个源文件，829 个单元测试，覆盖 42 个游戏子系统，整体完成度 **~92%**。
...
| **storage** | 2,953 行 | SQLite 持久化（14 张表）、仓库同步管理器、UPSERT 持久化、迁移框架、116 测试 |
...
| 模块 | 代码量 | 说明 |
|------|--------|------|
| **npc** | 1,146 行 | NPC 数据库（YAML 加载+硬编码回退）、商店/技能训练师/Warp/Quest/CashShop 交互、15 测试 |
| **script** | 942 行 | 脚本解析器（mes/next/close/select/warp/goto/set/goto_if）、变量系统、错误报告、17 测试 |

#### 仓库系统

| 模块 | 代码量 | 说明 |
|------|--------|------|
| **storage** | 2,953 行 | 仓库 CRUD、UPSERT 持久化、后台同步调度器（实际执行+超时恢复）、迁移框架、116 测试 |

#### 进阶系统（新增）

| 模块 | 代码量 | 说明 |
|------|--------|------|
| **homunculus** | 1,168 行 | 生命体系统：数据结构（六属性/技能/进化）、经验/升级/进化/喂食、持久化表、8 测试 |
| **mercenary** | 850 行 | 佣兵系统：数据结构（六属性/技能/忠诚度）、合同计时到期自动解散、5 种模板、6 测试 |
...
| 功能 | 说明 |
|------|------|
| Homunculus/Mercenary 持久化 | 迁移表已创建，管理器尚未对接数据库 |
| Mob/NPC YAML 完整解析 | 加载器已实现，需扩充 rAthena 数据映射 |
| MySQL 支持 | 迁移框架已就绪，需实现 MySQL 适配层 |
