...
> **92,330+ 行** Rust 代码，320+ 个源文件，1,369 测试（全部通过），覆盖 61 个服务端模块。
...
| 服务端代码 | 72,384 行 | 高质量 Rust 代码 |
| 客户端代码 | 7,600 行 | Bevy 游戏引擎 |
| 智能助手代码 | 1,537 行 | REPL + 知识库 |
| 总代码行数 | 92,330+ 行 | 包含测试和文档 |
| 脚本命令 | 580 个 | rAthena 兼容 95.7% |
...
| **unit** | 300+ 行 | 单位移动系统，碰撞验证（RealCollisionMap 集成），12 测试 |
| **intif** | 250+ 行 | 服务器间通信，消息队列，11 测试 |
| **zeny** | 120+ 行 | 金币管理，溢出保护 |
| **agent_api** | 180+ 行 | DeviAgent 服务端 API 接口 |
| **channel** | 450+ 行 | 公共/私人频道系统，支持玩家聊天、频道管理，7 测试 |
| **chrif** | 350+ 行 | 角色服务器接口，处理角色服务器与地图服务器通信，6 测试 |
| **clif** | 350+ 行 | 客户端接口，处理客户端与服务器通信，5 测试 |
| **pc** | 400+ 行 | 玩家角色详细机制，属性计算、状态管理，7 测试 |
| **ipban** | 250+ 行 | IP 封禁系统，暴力破解检测，3 测试 |
| **loginlog** | 200+ 行 | 登录日志系统，4 测试 |
| **char_logif** | 200+ 行 | 角色服务器日志接口，4 测试 |
...
| **script** | 4,000+ 行 | 脚本解析器（580 个命令，rAthena 兼容 95.7%）、表达式解析、函数调用栈、循环控制、背包/HP/广播/NPC变量集成、数组/字符串/数学运算、玩家/公会/队伍/地图/怪物/NPC/时间/状态/技能/装备/宠物/坐骑/任务/成就/公会战/交易/聊天/邮件/副本/战场/商城/Kafra/拍卖/摆摊/声望/名声/附魔/合成/造型师/摄像机/定时器/GM命令操作、98 测试 |
| **gat** | 386 行 | .gat 文件解析器（rAthena 格式 v1-5）、MapState 真实碰撞检测 |
| **migration** | 420 行 | 数据库迁移框架（schema_version 版本追踪、事务保护、幂等执行） |

### 客户端 (Devi) — 7,600+ 行，46 个源文件，80+ 测试
...
### 智能助手 (DeviAgent) — 1,537 行，16 个源文件
